...
// {Args: 5}
package origin.concurrency.restaurant2;
...
class Order { // (A data-transfer object)
  ...
  private final enumerated.menu.Food food;
  public Order(Customer cust, WaitPerson wp, enumerated.menu.Food f) {
    customer = cust;
    waitPerson = wp;
    food = f;
  }
  public enumerated.menu.Food item() { return food; }
...
class Plate {
  ...
  private final enumerated.menu.Food food;
  public Plate(Order ord, enumerated.menu.Food f) {
    order = ord;
    food = f;
  }
  public Order getOrder() { return order; }
  public enumerated.menu.Food getFood() { return food; }
...
class WaitPerson implements Runnable {
  ...
  public void placeOrder(Customer cust, enumerated.menu.Food food) {
    try {
  ...
  public void run() {
    try {
      while(!Thread.interrupted()) {
        ...
        enumerated.menu.Food requestedItem = order.item();
